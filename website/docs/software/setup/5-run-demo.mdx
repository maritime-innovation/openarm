...
If you want to run OpenArm with MoveIt2, start with fake hardware first.

1. Install ROS2 + MoveIt2 packages (Humble example):
...
2. Build the OpenArm ROS2 workspace:
...
3. Launch MoveIt2 demo:

```bash
ros2 launch openarm_bimanual_moveit_config demo.launch.py
```

### Real Hardware Operations (CAN + MoveIt2)

For real hardware operation, use these commands for safe restart and health checks.

1. Re-enable CAN interfaces:
...
ip -br link | grep -E 'can0|can1'
ip -details link show can0
ip -details link show can1
```

2. Restart MoveIt2 with log capture:

```bash
pkill -f "openarm_bimanual_moveit_config demo.launch.py" || true
...
mkdir -p ~/logs/openarm
LOG=~/logs/openarm/moveit_restart_$(date +%F_%H%M%S).log
ros2 launch openarm_bimanual_moveit_config demo.launch.py 2>&1 | tee "$LOG"
```

3. Verify hardware and controllers in another terminal:
...
4. Check common planning failures from logs:

```bash
grep -nE 'ERROR|WARN|Failed|failed|Socket|can0|can1|joint_states|ABORTED|No solution|start state|No acceleration limit' "$LOG"
```

### Known MoveIt2 Tuning Notes

- If planning fails with `Start state out of bounds`, check the corresponding joint limit in `openarm_description/config/arm/v10/joint_limits.yaml`.
- If planning fails with `No acceleration limit was defined`, enable acceleration limits in `openarm_bimanual_moveit_config/config/joint_limits.yaml`.

For more details, see [ROS2 Control and MoveIt2](../ros2/control).